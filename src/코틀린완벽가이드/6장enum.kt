...
/*
 ...
 4.데이터 클래스란 무엇인가 데이터 클래서에 대해 컴파일러가 자동으로 생성해주는 연산은 어떤게 있나,
   ...
   함수형 - 값지향 : 불변이고 다른 변수에 할당하면 새로 만들어져 복사된다(메모리에 들어있는 값으로 같은지를 판별)
   객체지만 불변객체는 값지향
...
data class D(val a:Int, val b:Int){
    operator fun component3():Int = 3 + 6
}
fun main(){
    val (d, b) = D(1,2)
    println(App.valueOf("A"))
    println(App.Companion.valueOf("B"))
}
