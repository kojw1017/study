...
/*
점화식을 세워보자
- 문자열의 길이를 `n`이라고 할 때, 자를 단위의 길이를 `k`라고 하자.
- `k`는 1부터 `n/2`까지의 값을 가질 수 있다 왜? 문자열의 절반보다 큰 단위로 자르면 압축이 불가능하니까
- 각 단위로 자를 때, 압축된 문자열의 길이를 계산하는 방법
  - 문자열을 `k` 길이로 자른다.
  - 연속된 같은 단위의 개수를 센다.
  - 압축된 문자열의 길이를 계산한다.
- 모든 `k`에 대해 압축된 문자열의 길이를 계산하고, 그 중 최소값을 찾는다.
 */
