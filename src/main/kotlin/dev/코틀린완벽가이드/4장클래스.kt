...
/*
    ...
    10. lazy 프로퍼티를 사용해 지연 계산을 달성하는 방법은 무엇인가? lazy와 lateinit 프로퍼티
    를 비교해보라
    lateinit : var 초기화 이후 계속 값이 바뀔수 있는경우, 기본타입에는 사용 불가
    di프레임워크나 어노테이션으로 확인
    lateinit 프레임워크나 라이프사이클에서 확실히 사용할 수있다면 사용 권장

    by lazy : val 초기화 이후 읽기 전용으로 사용할 경우

    11. 객체 선언이란 무엇인가? 코틀린 객체와 자바에서 일반적인 싱글턴 구현 패턴을 비교하라.
    자바에서 싱글턴 객체를 안전하게 만드는 유일한 방법은 Enum을 사용한다
    이유는 복잡한 sync를 걸지않고선 싱글턴객체 만들기 어렵다
